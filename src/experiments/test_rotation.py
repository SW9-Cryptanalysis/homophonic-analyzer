import numpy as np
from ..utils.embeddings import (
	normalize_vectors,
	build_anchor_matrices,
	solve_procrustes,
	find_closest,
	load_data_as_dict,
)
from ..utils.logging import get_colored_logger
from dotenv import load_dotenv
import os

load_dotenv()

logger = get_colored_logger("Rotation Test", int(os.getenv("LOGGING_LEVEL", 0)))

if __name__ == "__main__":
	generalized_model_vectors = load_data_as_dict(
		"embeddings/english_plaintext_embeddings.csv",
		lambda parts: np.array([float(x) for x in parts[1:]]),
	)

	generalized_model_vectors = normalize_vectors(generalized_model_vectors)

	cipher_model_vectors = load_data_as_dict(
		"embeddings/monoalphabetic-cipher_cipher_embeddings.csv",
		lambda parts: np.array([float(x) for x in parts[1:]]),
	)

	cipher_model_vectors = normalize_vectors(cipher_model_vectors)

	partial_key = [("e", "23"), ("t", "12")]

	logger.info("--- Starting Alignment Experiment ---")
	logger.info(f"Partial key: {partial_key}\n")

	# --- 2. Build Anchor Matrices ---
	try:
		X_anchors, Y_anchors = build_anchor_matrices(
			generalized_model_vectors, cipher_model_vectors, partial_key, logger,
		)
		logger.info(
			f"Built anchor matrices: X shape {X_anchors.shape},"
				f"Y shape {Y_anchors.shape}",
		)

		# --- 3. Solve for the Rotation Matrix W ---
		W = solve_procrustes(X_anchors, Y_anchors)
		logger.info(f"\nSolved for rotation matrix W (shape {W.shape}).")

		# --- 4. Define Keys to Attack ---
		known_source_symbols = {key for key, val in partial_key}
		unknown_source_keys = [
			key for key in generalized_model_vectors if key not in known_source_symbols
		]
		logger.info(f"Unknown source keys to find: {unknown_source_keys}")

		# --- 5. Define Search Space of Unknown Cipher Symbols ---
		known_cipher_symbols = {val for key, val in partial_key}
		search_space = {
			key: vec
			for key, vec in cipher_model_vectors.items()
			if key not in known_cipher_symbols
		}
		logger.info(
			f"Unknown cipher symbols to match against: {list(search_space.keys())}\n",
		)

		# --- 6. Loop and Generate Full Key ---

		predicted_key_map = {}

		logger.info("--- Running Attack Loop ---")
		for source_key in unknown_source_keys:
			if not search_space:
				logger.info("Warning: No more cipher symbols left to match.")
				break

			# Get the source vector
			source_vec = generalized_model_vectors[source_key]

			# Apply the Rotation
			rotated_vec = source_vec @ W
			rotated_vec /= np.linalg.norm(rotated_vec)  # Normalize

			# Find the best match *from the remaining symbols*
			predicted_cipher_key, similarity = find_closest(
				rotated_vec,
				search_space,
			)

			# Store the mapping
			predicted_key_map[source_key] = predicted_cipher_key

			logger.info(
				f"Plaintext '{source_key}' -> Cipher '{predicted_cipher_key}'"
				f"(Sim: {similarity:.4f})",
			)

			# Remove the matched key from the search space to ensure 1-to-1
			if predicted_cipher_key:
				del search_space[predicted_cipher_key]

		logger.info("\n--- FINAL PREDICTED KEY ---")
		# Add the known partial key to the map for a full view
		for src, tgt in partial_key:
			predicted_key_map[src] = tgt

		logger.info(sorted(predicted_key_map.items(), key=lambda x: x[0].lower()))

		# Check against our mock ground truth
		ground_truth = {
			v.lower(): k
			for k, v in load_data_as_dict(
				"embeddings/monoalphabetic-cipher_mappings.csv",
				lambda parts: parts[1],
			).items()
		}

		logger.info("\n--- GROUND TRUTH KEY ---")
		logger.info(sorted(ground_truth.items(), key=lambda x: x[0].lower()))
		correct_count = 0
		for key, val in predicted_key_map.items():
			if ground_truth.get(key) == val:
				correct_count += 1

		logger.info(f"\nAccuracy: {correct_count / len(ground_truth):.0%}")
		logger.info(f"Correct mappings: {correct_count} out of {len(ground_truth)}")

	except Exception as e:
		logger.info(f"\nAn error occurred: {e}")
		logger.info("This can happen if the partial key is empty or not found.")
